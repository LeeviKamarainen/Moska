...
async function initializeCode(gameArray) {
  ...
  if(gameArray.gamestates.length != 0) { 
    ...
    // Send 'renderingStopped' event, only if all players except one are finished in the last state.
    let nplayers_finished = gameArray.gamestates[gameArray.gamestates.length-1].players.filter(player => player.finished == true).length;
    console.log("nplayers_finished: "+nplayers_finished)
    if (nplayers_finished == 3) {
